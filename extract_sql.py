...
def get_patient_group_eicu(args, client):
    if args.patient_group != 'Generic':
        query = \
            """
            SELECT i.patientunitstayid, i.gender, i.age, i.ethnicity AS race,  
...
def query_anti_eicu(client, icuids_to_keep, tw_in_minutes):
    query = \
        ...
        WHERE (REGEXP_CONTAINS(lower(drugname), r"^.*adoxa.*$")
          ...
          OR REGEXP_CONTAINS(lower(drugname), r"^.*oethnicity AS racea.*$")
          ...
          OR REGEXP_CONTAINS(lower(drugname), r"^.*spectethnicity AS racef.*$")
